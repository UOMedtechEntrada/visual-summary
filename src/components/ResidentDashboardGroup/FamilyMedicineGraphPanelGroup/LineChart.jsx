...
export default (props) => {
    ...
    const d3Line = line().x((d) => d.x).y((d) => d.y),
        elementSize = props.smallScreen ? 3 : 6.5,
        elementList = _.map(props.data, (d, i) => <circle
            id={'point-inner-' + props.epaIDClass + '-outer-' + d.recordID}
    ...
    return (
        <svg height={props.innerHeight + 100} width={props.width} className='score-svg' >
            ...
            <g>{elementList}</g>
            <g>
                {_.map(props.scoreScale, (s, si) => {
                    return <text key={'scale-reference-' + si} x={40} y={(props.innerHeight + 20) + (si * 20)}>{si + 1} - {s}</text>
                })}
            </g>
        </svg>)
}
